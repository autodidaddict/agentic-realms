...
- [x] T040 Run `Reconciler` as a **Horde cluster singleton**: add `NpcMinds.Registry` (`Horde.Registry`) in `lib/agenticrealms/npc_minds/registry.ex` and `NpcMinds.Supervisor` (`Horde.DynamicSupervisor`, `ensure_reconciler/0`) in `lib/agenticrealms/npc_minds/supervisor.ex`; register the reconciler under the Horde registry via-name; wire Registry + Supervisor + an idempotent boot starter into `lib/agenticrealms/application.ex` (one instance cluster-wide, relocated by Horde on node loss).
